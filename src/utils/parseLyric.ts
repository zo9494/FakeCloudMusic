...
export function parseLyric(
  ...
  if (dynamic.trim().length === 0) {
    ...
    parsePureLyric(translated).forEach(line => {
      ...
      if (target) {
        target.tlyric = target.tlyric || '';
        if (target.tlyric.length > 0) {
          target.tlyric += ' ';
        }
        target.tlyric += line.lyric;
      }
    ...
    for (let i = 0; i < processed.length; i++) {
      ...
      if (
        thisLine &&
        nextLine &&
        thisLine.lyric.trim().length > 0 &&
        nextLine.lyric.trim().length > 0 &&
        thisLine.duration > 0
      ) {
...
export function processLyric(lyric: LyricLine[]): LyricLine[] {
  ...
  lyric.forEach((thisLyric, i, lyric) => {
    if (thisLyric.lyric.trim().length === 0) {
